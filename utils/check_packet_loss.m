function [data] = check_packet_loss(log_name)
    ...
    % packet loss
    resulting_vector = zeros(size(packetRecv));
    error_indices = packetRecv ~= 1;
    resulting_vector(error_indices) = 1;
    packetLoss = [resulting_vector(2: end); 0];
    % Plot the time series
    figure()
    plot(timeVec, packetRecv);
    data = [timeVec, packetRecv, packetLoss];
end