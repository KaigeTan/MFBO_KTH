...
remap=(1,0,2,0,1,0)
foot = bytearray([92,85,83,97,93,85,92,84,87,86,90,80])

# TCP Transmission
HOST0 = "10.2.58.100"#'172.16.3.104'
PORT0 = 8888
...
#         self.client_socket, _ = self.tcp.accept()
        print('Main start ...')
        logging.info("start!")
        self.client_socket.connect(self.data.tcp_host_port)
        timer_com_stm32 = perpetualTimer(0.05, self.Write_Read_STM32_thread,'Write_Read_STM32')
        timer_write_simulink = perpetualTimer(0.05, self.tcp_Transmit, 'tcp_Transmit')
    ...
    def tcp_Transmit(self):
        ...
        self.sensor_data.acc = self.imu.linear_acceleration
...
#             self.tcp.bind(self.data.tcp_host_port)
            ...
            logging.error('Socket to simulink setup failed! Error: %s', str(error))
