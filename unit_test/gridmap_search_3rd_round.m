...
T_vec = (0.9-0.5)*round(base_vec, 2) + 0.5;        % oscillation period, 0.5 - 0.9 s
alpha_vec = (0.88-0.8)*round(base_vec, 2) + 0.8;   % shape ratio, determines the time of rising phase (var), 0.8 - 0.88
% from oscillator to the actuation signal
alpha_b_gain_vec = (0.7-0.60)*round(base_vec, 2) + 0.60;    % maximal bending angle, 0.63 - 0.7 rad
z_l_gain_vec = (0.008-0.0072)*round(base_vec, 4)+ 0.0072;    % maximal bending length, 0.0072 - 0.008 m
z_l_diff_vec = (4-3)*round(base_vec, 4) + 3;             % phase difference between 3 - 4 rad
...
t_cost = toc;
% save results
file_name = [parentFolder, '\data\exhausted_search\3rd_grid_search_5DOF_5pts_', 'time', num2str(t_cost), '.mat'];
